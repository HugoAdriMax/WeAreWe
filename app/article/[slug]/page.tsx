...
// Définition de l'interface pour l'article
interface Article {
  title: string;
...
export default function ArticlePage() {
  ...
  const slug = params?.slug; // Récupérer le slug des paramètres de l'URL
  ...
  const [showShareMenu, setShowShareMenu] = useState(false); // État pour le menu de partage

  // Fonction pour charger l'article en fonction du slug
  useEffect(() => {
    if (!slug) return;
    ...
    async function fetchArticle() {
      try {
        ...
        setArticle(data[0]); // Enregistrer l'article dans l'état
      } catch (e) {
  ...
  // Fonction de partage de l'article
  ...
  const copyToClipboard = () => {
    ...
    // Possibilité d'ajouter une notification ici
  };
